...
class BK:
    ...
    def run(self):
        while True:
            ...
            if choice == '1':
                BK.create_account(self)
            elif choice == '2':
                card = input('Enter your card number:\n')
                pin = input('Enter your PIN:\n')
                if BK.check_account(self, card, pin):
    ...
    def transfer(self, from_, to):
        ...
        elif not BK.check_receiver(self, to):
            print('Probably you made mistake in the card number. Please try again!')
        elif not BK.check_card(self, to):
            print("Such a card does not exist.")
        else:
            amount = int(input('Enter how much money you want to transfer:'))
            if amount > BK.get_balance(self, from_):
                print('Not enough money!')
            else:
                BK.add_income(self, to, amount)
                BK.withdraw(self, from_, amount)
                print('Success!')
    ...
    def login(self, card_number):
        while True:
            ...
            if choice == '1':
                print('Balance:', BK.get_balance(self, card_number))
            elif choice == '2':
                amount = int(input('Enter income:'))
                BK.add_income(self, card_number, amount)
            elif choice == '3':
                receiver = input('Enter card number:')
                BK.transfer(self, card_number, receiver)
            elif choice == '4':
